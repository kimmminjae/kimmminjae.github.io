## This script has minor modifications to Jeremy Davis’ blog post about his Low-Effort Solr Install script.
## Check out his original script at https://jermdavis.wordpress.com/2017/10/30/low-effort-solr-installs/

Param(
    $solrVersion = "7.2.1",
    $installFolder = "c:\sitecore\solr",
    $solrPort = "8983",
    $solrHost = "solr",
    $solrSSL = $true,
    $nssmVersion = "2.24",
    $JREVersion = (gci "c:\Program Files\Java\" | ? { $_.PSIsContainer } | sort CreationTime)[-1].toString()
)

$JREPath = "C:\Program Files\Java\$JREVersion" ## Note that if you're running 32bit java, you will need to change this path
